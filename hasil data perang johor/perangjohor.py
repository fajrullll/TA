...
import networkx as nx                  # <-- DITAMBAHKAN
from scipy.spatial import ConvexHull   # <-- DITAMBAHKAN
import os
...
def cek_nun(seg_mask, h_seg, w_seg, posisi_titik, scale):
    # 1. Perlebar batas tinggi ganda untuk mengakomodasi ekor mangkuk yang melengkung ke atas
    if h_seg >= 80 * scale: return False 
    
    # 2. Karakteristik utama mangkuk: Proporsi lebar yang hampir sama atau lebih dari tingginya
    if w_seg > h_seg * 0.95 and w_seg > 20 * scale: 
        return True
        
    # 3. Jika fisiknya cukup lebar dan anatomi ujung atasnya maksimal 2 (pangkal dan ekor)
    if w_seg >= 25 * scale:
        gigi_atas = analisis_ujung_atas(seg_mask)
        if gigi_atas <= 2:
            return True
            
    # 4. Fallback jika secara default sistem biner awal sudah memetakan 1 titik padanya
    if posisi_titik == "ATAS_SINGLE": 
        return True
...
def potong_persimpangan(labeled_letters, skeleton_used, dot_mask, scale):
    ...
    for region_label in range(1, num_features + 1):
        ...
        for cycle, _ in fitur_kumpulan:
            cycle_arr = np.array(cycle)
            cycle_bounds.append((cycle_arr[:, 1].min(), cycle_arr[:, 1].max()))
        ...
        if cut_points:
            ...
            valid_cuts = sorted([int(np.clip(c, minc, maxc)) for c in cut_points])
            
            final_valid_cuts = []
            for c in valid_cuts:
                if final_valid_cuts and c - final_valid_cuts[-1] < 15 * scale:
                    continue 
        ...
        for i in range(len(cuts) - 1):
            ...
            if np.sum(segment_mask) > 30 * (scale**2) and segment_width > 5 * scale:
                ...
                elif cek_nun(segment_mask, h_seg, w_seg, posisi_titik, scale) or (w_seg > h_seg * 1.3 and not has_fitur): tipe_huruf = "NUN"
                ...
                elif cek_dal(segment_mask, h_seg, w_seg, scale): tipe_huruf = "DAL"
                elif cek_ain(segment_mask, h_seg, w_seg, posisi_titik, scale): tipe_huruf = "AIN"
                ...
                elif cek_fa(segment_mask, h_seg, w_seg, posisi_titik, scale): tipe_huruf = "FA"
        ...
        # --- TAHAP MATRIKS JARAK & KUOTA ---
        kuota_titik = {}
        for idx, meta in enumerate(segment_metadata):
            th = meta.get('tipe_huruf', 'UNKNOWN')
            if th in ["NUN", "FA", "BA", "DAL", "RA", "WAW"]: 
                kuota_titik[idx] = 1
            elif th in ["YA"]:
                kuota_titik[idx] = 2
            else:
        ...
        for dot_idx, dot_prop in enumerate(dots_props):
            ...
            if not (grp_min_r - (150 * scale) <= dy <= grp_max_r + (200 * scale) and grp_min_c - (100 * scale) <= dx <= grp_max_c + (100 * scale)):
                continue
            ...
            for cand_idx, seg_mask in enumerate(clean_segments):
                ...
                has_fitur = meta.get('has_fitur', False)
                
                if has_fitur:
                    target_y, target_x = meta['fitur_cy'], meta['fitur_cx']
                else:
                    target_y, target_x = seg_cy, seg_cx

                # --- KOREKSI ILUSI SPASIAL CENTROID ---
                seg_crds = np.argwhere(seg_mask)
                if len(seg_crds) > 0:
                    min_x_seg = seg_crds[:, 1].min()
                    max_x_seg = seg_crds[:, 1].max()
                    
                    if tipe_huruf == "NUN":
                        # Magnet NUN difokuskan ke pangkal gigi (kanan) bukan tengah mangkuk
                        target_x = max_x_seg - ((max_x_seg - min_x_seg) * 0.3)
                    elif tipe_huruf == "AIN":
                        # Titik NGA meluber ke kiri, fokuskan magnet ke kiri
                        target_x = min_x_seg + ((max_x_seg - min_x_seg) * 0.2)
                else:
                    min_x_seg, max_x_seg = 0, 0
                # --------------------------------------

                dist_x_target = abs(dx - target_x)
                dist_y_target = abs(dy - target_y)
                dist_real = euclidean((dy, dx), (target_y, target_x))
                
                tolerance_kiri = 25 * scale
                tolerance_kanan = 25 * scale
                if tipe_huruf == "AIN":
                    tolerance_kiri = 100 * scale 
                    
                is_within_bounds = (min_x_seg - tolerance_kiri <= dx <= max_x_seg + tolerance_kanan)
                
                if tipe_huruf in ["LAM", "WAW", "ALIF"] and not has_fitur:
                    continue 

                if not is_within_bounds:
                    score = 50000 + dist_x_target + dist_y_target
                else:
                    if has_fitur:
                        if dy < target_y and dist_x_target < 60 * scale: score = -20000 + dist_x_target 
                        else: score = (dist_x_target * 2.0) + dist_y_target
                    elif tipe_huruf == "AIN":
                        if dy < target_y and dist_x_target < 120 * scale: 
                            score = -30000 + dist_x_target 
                        else: score = (dist_x_target * 2.0) + dist_y_target 
                    elif tipe_huruf in ["YA", "BA"]:
                        if dy > target_y and dist_x_target < 40 * scale: score = -10000 + dist_x_target 
                        else: score = (dist_x_target * 2.0) + dist_y_target 
                    elif tipe_huruf in ["NUN", "SIN"]:
                        # Dengan target_x yang baru, jarak ke titik sendiri (Nun) akan sangat kecil (< 50 scale)
                        # Jarak ke titik tetangga (Nga) akan melebar (> 50 scale), sehingga ditolak ke penalti
                        if dy < target_y and dist_x_target < 50 * scale: 
                            score = -25000 + dist_x_target
                        else: score = (dist_x_target * 2.0) + dist_y_target
                    else:
                        score = dist_x_target + (dist_y_target * 0.8)

                is_win_via_magnet = (score < -5000)
                valid_attach = False
                
                if is_win_via_magnet:
                    valid_attach = True
                elif tipe_huruf in ["YA", "BA"] and (dy < target_y or dist_x_target > 40 * scale):
                    valid_attach = False
                elif tipe_huruf in ["NUN", "SIN"] and (dy > target_y or dist_x_target > 50 * scale):
                    valid_attach = False
                elif tipe_huruf == "AIN" and (dy > target_y or dist_x_target > 120 * scale): 
                    valid_attach = False
                elif has_fitur and (dy > target_y or dist_x_target > 60 * scale):
                    valid_attach = False
                elif dist_real < 90 * scale and dist_x_target < 50 * scale:
                    valid_attach = True
        ...
        kandidat_pasangan.sort(key=lambda x: (x['score'], x['dist']))
        
        titik_terpakai = set()
        alokasi_saat_ini = {idx: 0 for idx in range(len(clean_segments))}
        
        for pas in kandidat_pasangan:
            d_idx = pas['dot_idx']
            c_idx = pas['cand_idx']
            
            if d_idx in titik_terpakai: continue
            if alokasi_saat_ini[c_idx] >= kuota_titik[c_idx]: continue
                
            dot_indices = (labeled_dots == pas['prop'].label)
            clean_segments[c_idx] = np.logical_or(clean_segments[c_idx], dot_indices)
            
            titik_terpakai.add(d_idx)
            alokasi_saat_ini[c_idx] += 1
