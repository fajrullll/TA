...
def visualisasi_freeman(debug_records, label_step=4, target_xrange_visual=None):
    ...
    for idx, rec in enumerate(debug_records):
        ...
        # DEBUG CONSOLE
        # ======================================================
        print("\n========== FREEMAN CODE ARAH GORESAN ==========")
        print(f"Region                    : {rec.get('region_label', idx + 1)}")
        print(f"Cut column                : {cut_col}")
        print(f"Target xrange visual      : {target_xrange_visual}")
        print(f"Jumlah titik path asli    : {len(path_asli)}")
        print(f"Jumlah titik path visual  : {len(visual_indices)}")
        print(f"Jumlah kode arah goresan  : {len(codes_arah_tulis)}")
        print(f"Code arah goresan         : {codes_arah_tulis.tolist()}")
        print("Normalisasi               : path dibalik, lalu code = (code + 4) mod 8")
