...
def cek_ra(seg_mask, h_seg, w_seg):
    if h_seg >= 55: return False
    if w_seg < 8 or w_seg > 45: return False 
    skel = skeletonize(seg_mask)
    loops = deteksi_loop(skel)
    ...
    if len(endpoints) >= 2:
        ...
        # Pastikan arah kemiringan diagonal ke kiri
        if top[1] > bottom[1] - 10: 
            coords = np.argwhere(seg_mask)
            lowest_y = np.max(coords[:, 0]) 
            # Pastikan ujung bawah adalah titik terendah dari seluruh badan
            if abs(bottom[0] - lowest_y) <= 15: return True
    return False
...
def cek_ya(seg_mask, h_seg, w_seg, posisi_titik):
    ...
    if w_seg < 35 and h_seg < 35: return True  
    gigi_atas = analisis_ujung_atas(seg_mask)
    return gigi_atas <= 1
    ...
    for region_label in range(1, num_features + 1):
        ...
        for i in range(len(cuts) - 1):
            ...
            if np.sum(segment_mask) > 30 and segment_width > 5:
                ...
                if cek_lam(h_seg, w_seg):
                    ...
                    tipe_huruf = "WAW"
                elif cek_ra(segment_mask, h_seg, w_seg): # FUNGSI RA AKTIF SEBAGAI JURI
                    tipe_huruf = "RA"
        ...
        for dot_prop in dots_props:
            ...
            for idx, seg_mask in enumerate(clean_segments):
                ...
                # RA DITAMBAHKAN KE DAFTAR PENOLAK TITIK (DESTROYER)
                if tipe_huruf in ["SIN", "LAM", "HA", "WAW", "ALIF", "RA"]:
                    continue 
...
for idx, region_mask in enumerate(sorted_hasil_potongan):
    ...
    for segment in tsp_segments:
        ...
        ax[1].scatter(segment[:, 1], segment[:, 0], color='white', s=5, zorder=5)
