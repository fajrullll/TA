...
def cek_apakah_tiang_signifikan(skel, start_yx, min_height=15, max_width_spread=20, debug=False):
    """
    ...
    for (ny, nx) in neighbors:
        ...
        # Jika lolos tinggi minimum, cek rasio kelangsingan
...
def hitung_garis_potong_sinkron(component_features, labeled_letters, skeleton_used, min_gap=3):
    ...
    for label, feat in component_features.items():
        ...
        # Agar kepala Mim/Sad/Fa tidak terpotong di awal
        ...
        if not skip_start:
            # Potong di awal grup pertama (standard separation dari kiri)
        ...
        for i in range(len(intersections) - 1):
            ...
            for y_check in ys:
                if cek_apakah_tiang_signifikan(letter_skeleton, (y_check, curr_x)):
                    is_curr_tiang = True
                    break
...
def potong_persimpangan(labeled_letters, skeleton_used):
    hasil_potongan = []
    ...
    for region_label in range(1, num_features + 1):
        ...
        if raw_intersections:
            ...
            for i in range(len(raw_intersections) - 1):
                ...
                if is_curr_tiang:
                    # Tiang -> Potong Tengah (Pisahkan Lam dari Sin)
                    cut_points.append((curr_x + next_x) // 2)
                elif gap < threshold_gigi:
                    # Gigi Sin -> Skip (Lindungi)
                    continue
                else:
                    # Jarak Jauh -> Potong Normal (jika aman dari loop)
                    skip_cut = False
                    if loops:
                        loop_xs = [np.mean(np.array(cycle), axis=0)[1] for cycle, _ in loops]
                        if loop_xs:
...
for label, props in component_features.items():
    ...
    for iy, ix in props["intersections"]:
        ...
        # Panggil cek_apakah_tiang_signifikan dengan mode DEBUG
        # Ini akan mengembalikan area yang dianggap tiang (Cyan)
        is_tiang, visited_pixels, bbox = cek_apakah_tiang_signifikan(skel, (iy, ix), debug=True)
...
# 3. Plot Garis Potong Merah (SINKRON)
