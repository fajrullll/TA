...
def dominasi_diakritik(min_x, max_x, dot_mask):
    ...
    if lebar_titik > 20: return f"{posisi}_DOUBLE"
...
def cek_alif(seg_mask, h_seg, w_seg, posisi_titik):
    # ALIF: Tinggi, sempit, TANPA titik, TANPA loop. (Pemisah ALIF dan YA')
    if posisi_titik != "TIDAK_ADA": return False
    if h_seg >= 30 and w_seg <= 25:
        skel = skeletonize(seg_mask)
...
def cek_waw(seg_mask, h_seg, w_seg):
    if h_seg >= 60: return False
    if w_seg > 60: return False # Lebar harus pendek! (Mencegah gabungan Ya'-Ha' masuk)
    ...
    if len(loops) == 1:
        ...
        if 1 <= len(endpoints) <= 2:
            # Pastikan loop berada di setengah atas badan
            cycle_arr = np.array(loops[0][0])
            loop_min_y = np.min(cycle_arr[:, 0])
            coords = np.argwhere(seg_mask)
            top_y = np.min(coords[:, 0])
            if loop_min_y - top_y < 20: return True
    return False

def cek_ba(seg_mask, h_seg, w_seg, posisi_titik):
    if h_seg >= 60: return False
    skel = skeletonize(seg_mask)
...
def cek_ya(seg_mask, h_seg, w_seg, posisi_titik):
    ...
    if len(intersections) > 0 and h_seg < 40: return True
    if posisi_titik == "BAWAH_DOUBLE": return True
    if posisi_titik == "BAWAH_SINGLE": return False
    if w_seg < 35 and h_seg < 35: return True  # Gigi Ya pendek
...
def hitung_garis_potong_sinkron(component_features, labeled_letters, skeleton_used, min_gap=3):
    ...
    for label, feat in component_features.items():
        ...
        # --- PRE-CLASSIFICATION SHIELD (BYPASS PEMOTONGAN) ---
        ...
        # JIKA Waw Murni, Batal Potong!
        if cek_waw(mask, h_grp, w_grp) or cek_ha(mask, h_grp, w_grp):
            continue 
        # -----------------------------------------------------
        ...
        if loops:
            ...
            # Toleransi gentle kembali ke 5 pixel agar sambungan Ya-Ha bisa dipotong
            if loop_xs and abs(first_ix - loop_xs[0]) < 5: skip_start = True
...
def potong_persimpangan(labeled_letters, skeleton_used, dot_mask):
    ...
    for region_label in range(1, num_features + 1):
        ...
        # --- PRE-CLASSIFICATION SHIELD (BYPASS PEMOTONGAN) ---
        if cek_waw(mask_group, h_grp, w_grp) or cek_ha(mask_group, h_grp, w_grp):
            pass # Biarkan cut_points kosong! Pisau pemotong disarungkan.
        else:
        ...
        for i in range(len(cuts) - 1):
            ...
            if np.sum(segment_mask) > 30 and segment_width > 5:
                ...
                if cek_lam(h_seg, w_seg):
                    tipe_huruf = "LAM"
                elif cek_alif(segment_mask, h_seg, w_seg, posisi_titik):
                    tipe_huruf = "ALIF"
                elif cek_sin(segment_mask, h_seg):
                    tipe_huruf = "SIN"
                elif cek_nun(segment_mask, h_seg, w_seg, posisi_titik):
                    tipe_huruf = "NUN"
                elif cek_ha(segment_mask, h_seg, w_seg): 
                    tipe_huruf = "HA"
                elif cek_waw(segment_mask, h_seg, w_seg): 
                    tipe_huruf = "WAW"
                elif cek_ba(segment_mask, h_seg, w_seg, posisi_titik): 
                    tipe_huruf = "BA"
        ...
        for dot_prop in dots_props:
            ...
            for idx, seg_mask in enumerate(clean_segments):
                ...
                if tipe_huruf in ["YA", "BA"]:
                    if dy > seg_cy and dist_x_centroid < 40: 
            ...
            if best_candidate is not None:
                ...
                if is_win_via_magnet:
                    valid_attach = True
                elif tipe_huruf_winner in ["YA", "BA"] and (dy < win_cy or dist_x_winner > 40):
                    valid_attach = False
                elif tipe_huruf_winner == "NUN" and (dy > win_cy or dist_x_winner > 40):
                    valid_attach = False
                elif dist_real < 90 and dist_x_winner < 50:
                    valid_attach = True
...
image_path = r"E:\progres\fgrtry7.png"
...
for idx, region_mask in enumerate(sorted_hasil_potongan):
    ...
    if cek_lam(h_seg, w_seg):
        tipe_huruf = "LAM"
    elif cek_alif(badan_huruf, h_seg, w_seg, posisi_titik):
        tipe_huruf = "ALIF"
    elif cek_sin(badan_huruf, h_seg):
        tipe_huruf = "SIN"
    elif cek_nun(badan_huruf, h_seg, w_seg, posisi_titik):
        tipe_huruf = "NUN"
    elif cek_ha(badan_huruf, h_seg, w_seg): 
        tipe_huruf = "HA"
    elif cek_waw(badan_huruf, h_seg, w_seg): 
        tipe_huruf = "WAW"
    elif cek_ba(badan_huruf, h_seg, w_seg, posisi_titik): 
        tipe_huruf = "BA"
    elif cek_ya(badan_huruf, h_seg, w_seg, posisi_titik):
        tipe_huruf = "YA"

    # --- PENGGAMBARAN KOTAK BOUNDING BOX ---
    if tipe_huruf == "YA":
        ...
        rect = patches.Rectangle((min_x_seg, min_y_seg), w_seg, h_seg, linewidth=2, edgecolor='red', facecolor='none', linestyle='-')
        ax.add_patch(rect)
        ax.text(min_x_seg, min_y_seg-10, f"BA' (H{idx+1})", color='red', fontsize=10, fontweight='bold')
    elif tipe_huruf == "SIN":
        ...
        rect = patches.Rectangle((min_x_seg, min_y_seg), w_seg, h_seg, linewidth=2, edgecolor='violet', facecolor='none', linestyle='-.')
        ax.add_patch(rect)
        ax.text(min_x_seg, min_y_seg-10, f"HA' (H{idx+1})", color='violet', fontsize=10, fontweight='bold')
    elif tipe_huruf == "WAW":
        rect = patches.Rectangle((min_x_seg, min_y_seg), w_seg, h_seg, linewidth=2, edgecolor='cyan', facecolor='none', linestyle='-')
        ax.add_patch(rect)
        ax.text(min_x_seg, min_y_seg-10, f"WAW (H{idx+1})", color='cyan', fontsize=10, fontweight='bold')
    elif tipe_huruf == "ALIF":
        rect = patches.Rectangle((min_x_seg, min_y_seg), w_seg, h_seg, linewidth=2, edgecolor='white', facecolor='none', linestyle=':')
        ax.add_patch(rect)
        ax.text(min_x_seg, min_y_seg-10, f"ALIF (H{idx+1})", color='white', fontsize=10, fontweight='bold')
